									...
									<div class="pull-right">
										{if $EXTENSION->isVtigerCompatible()}
											...
											{else}
												...
												{else}
													<button class="oneclickInstallPaid btn btn-info btn-lg {if $IS_AUTH}authenticated {else} loginRequired{/if}" data-trial=false>{vtranslate('LBL_BUY',$QUALIFIED_MODULE)}${$EXTENSION->get('price')}</button>   
												{/if}
											{/if}
										{else}
